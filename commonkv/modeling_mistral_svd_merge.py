...
class MistralAttention(nn.Module):
    ...
    def __init__(self, config: MistralConfig, layer_idx: Optional[int] = None):
        ...
        self.k_up_proj = nn.Linear(self.hidden_size, self.k_rank, bias=False)
        self.k_down_proj = nn.Linear(self.k_rank, self.num_key_value_heads * self.head_dim, bias=False)
        self.v_up_proj = nn.Linear(self.hidden_size, self.v_rank, bias=False)
        self.v_down_proj = nn.Linear(self.v_rank, self.num_key_value_heads * self.head_dim, bias=False)
    ...
    def forward(
        ...
        if past_key_value is not None:  # 暂时注释
...
class MistralModel(MistralPreTrainedModel):
    ...
    def merge_all_kv(self, past_key_values):
        ...
        for group_idx in range(num_groups):
            ...
            batch_indices = torch.arange(batch_size, device=device).view(-1, 1, 1).expand(-1, num_heads,
                                                                                          seq_len).flatten()
            head_indices = torch.arange(num_heads, device=device).view(1, -1, 1).expand(batch_size, -1,
                                                                                        seq_len).flatten()
            seq_indices = torch.arange(seq_len, device=device).view(1, 1, -1).expand(batch_size, num_heads,
                                                                                     -1).flatten()
        ...
        layer_avg_weight = [0.5725364248599604, 0.19424219990970432, 0.09093340055568955, 0.1422879746746457,
                            0.43200904319032596, 0.3947568807210229, 0.10334285062348725, 0.06989122546516391,
                            0.20709781661679047, 0.4044973090158895, 0.19104355402219808, 0.197361320345122,
                            0.18819994365156287, 0.25496593620350255, 0.2948229413024837, 0.2620111788424509,
                            0.1890419936170194, 0.19674429336996369, 0.3352710482723522, 0.2789426647406647,
                            0.30918022400924905, 0.23548598534472787, 0.2090546803032022, 0.24627911034282085,
                            0.21401715471007046, 0.3733325443246397, 0.12106487104898705, 0.29158542991630276,
                            0.23478604583156762, 0.31003491763749597, 0.15658168967662153, 0.2985973468543149]
